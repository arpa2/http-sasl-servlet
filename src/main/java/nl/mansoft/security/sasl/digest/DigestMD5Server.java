...
package nl.mansoft.security.sasl.digest;
 ...
 * @author Rosanna Lee
 */
final class DigestMD5Server extends DigestMD5Base implements SaslServer, Externalizable {
  ...
  public DigestMD5Server() throws SaslException {
    super(null, MY_CLASS_NAME, 1, null, null);
  }
  ...
  private static void writeByteArray(ObjectOutput out, byte[] barr) throws IOException {
    if (barr == null) {
      out.writeInt(-1);
    } else {
  ...
  private static byte[] readByteArray(ObjectInput in) throws IOException {
  ...
  public void writeExternal(ObjectOutput out) throws IOException {
    ...
    out.writeBoolean(integrity);
    writeByteArray(out, qop);
    out.writeByte(allQop);
    writeByteArray(out, strength);
    out.writeInt(sendMaxBufSize);
    out.writeInt(recvMaxBufSize);
    out.writeInt(rawSendSize);
    out.writeObject(myClassName);
    // DigestMD5Base
    out.writeInt(step);
    out.writeObject(cbh);
    //out.writeObject(secCtx);
    writeByteArray(out, H_A1);
    writeByteArray(out, nonce);
    out.writeObject(negotiatedStrength);
    out.writeObject(negotiatedCipher);
    out.writeObject(negotiatedQop);
    out.writeObject(negotiatedRealm);
    out.writeBoolean(useUTF8);
    out.writeObject(encoding.name());
    out.writeObject(digestUri);
    out.writeObject(authzid);
    // DigestMD5Server
    out.writeObject(specifiedQops);
    writeByteArray(out, myCiphers);
    out.writeObject(serverRealms);
  }
  ...
  public void readExternal(ObjectInput in) throws IOException, ClassNotFoundException {
    ...
    integrity = in.readBoolean();
    qop = readByteArray(in);
    allQop = in.readByte();
    strength = readByteArray(in);
    sendMaxBufSize = in.readInt();
    recvMaxBufSize = in.readInt();
    rawSendSize = in.readInt();
    myClassName = (String) in.readObject();
    // DigestMD5Base
    step = in.readInt();
    cbh = (CallbackHandler) in.readObject();
    //secCtx = (SecurityCtx) in.readObject();
    H_A1 = readByteArray(in);
    nonce = readByteArray(in);
    negotiatedStrength = (String) in.readObject();
    negotiatedCipher = (String) in.readObject();
    negotiatedQop = (String) in.readObject();
    negotiatedRealm = (String) in.readObject();
    useUTF8 = in.readBoolean();
    encoding = Charset.forName((String) in.readObject());
    digestUri = (String) in.readObject();
    authzid = (String) in.readObject();
    // DigestMD5Server
    specifiedQops = (String) in.readObject();
    myCiphers = readByteArray(in);
    serverRealms = (List<String>) in.readObject();
  }
}
